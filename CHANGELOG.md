...
- 发布流程加固：`prepublishOnly` 不再依赖 Playwright 浏览器，新增 `verify:release` 完整校验脚本；发布工作流先执行完整校验（含 Chromium E2E）再发布，npm 凭据缺失时显式失败；CI 增加质量闸门聚合任务，Node 版本统一为 22。
- 修复 Web UI 内嵌脚本因字符串字面量跨行导致的整页脚本语法错误，恢复模型列表加载与页面交互。
...
- 为 LLM、技能评测、目录批处理、Web 请求和 MCP 工具补充取消信号、超时、Token/字符上限及调用预算；MCP 支持取消通知。
